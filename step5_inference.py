...
if __name__ == "__main__":
    tflitepath = './output6_6/models/Audio2Face.tflite'
    model_path = './output6_6/models/Audio2Face'
    
    inference = tfliteInference(tflitepath,model_path)
    data = np.load(os.path.join('./lpc/1114_3_11.npy'))

    weight = inference.get_weight(data)    
    print(weight.shape)

    np.set_printoptions(threshold=np.inf)
    # with open('./weight3.txt','w') as f:
    #     for i in range(weight.shape[0]):
    #         f.write(str(weight[i])+'\n')
    #     print(f'Write weight successfully!')
    # 保存为npy文件
    np.save('./weight5.npy', weight)


    # 加载npy文件
    weight = np.load('./weight5.npy')

    # 保存为txt文件
    np.savetxt('./weight5.txt', weight)
